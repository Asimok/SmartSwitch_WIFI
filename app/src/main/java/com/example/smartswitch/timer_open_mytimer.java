...
public class timer_open_mytimer extends AppCompatActivity implements NumberPicker.OnValueChangeListener, View.OnClickListener {
    public final UUID MY_UUID = UUID
            .fromString("00001101-0000-1000-8000-00805F9B34FB");
    public byte[] LED_STATE_TIMER_OPEN = {(byte) 0xff, 0x02, 0x00, 0x0A, (byte) 0xfe};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.timeropen);
    ...
    public void onValueChange(NumberPicker picker, int oldVal, int newVal) {
        ...
        Log.d("aa", "dateToString.nowdateToString4()   " + dateToString.nowdateToString4());

        //  tvstartTime.setText(Hour + " 时 " + Minute + " 分 " + Second + " 秒");
        setTitle("开启时间:  " + Hour + " 时 " + Minute + " 分 " + Second + " 秒");
        /*
         * 例子 生成16进制 HH MM SS
         *
         * HH=hexStringToBytes(Hour)[0]  //因为hexStringToBytes() 方法返回的是数组 所以取[0]
         * 动态生成指令  并不在此处 只做演示
         * LED_STATE_TIMER_OPEN=TimeClose.OneDayClose("1", HH, MM,SS);
         *
         * */
//        Log.d("aa", "(Hour)  " + Hour);
//        Log.d("aa", "hexStringToBytes(Hour)  " + hexStringToBytes(Hour));//将字符串转为字节
//        Log.d("aa", "byte2HexStr(hexStringToBytes(Hour))  " + byte2HexStr(hexStringToBytes(Hour))); //将字节转为16进制字符串
//        byte cc = TimeClose.OneDayClose("1", (byte) 0x00, (byte) 0x00, hexStringToBytes(Hour)[0])[6];
//        Log.d("aa", "(新添加的变量)  " + cc);
//        Log.d("aa", "(周一定时指令)  " + byte2HexStr(TimeClose.OneDayClose("1", (byte) 0x00, (byte) 0x00, hexStringToBytes(Hour)[0])));
        ...
        switch (v.getId()) {
            case R.id.ck7: {
                if (date.equals("000000")) {
                    Toast.makeText(this, "请设置开灯时间  ", Toast.LENGTH_LONG).show();
                } else {
                    open.setEnabled(true);
            ...
            case R.id.ckr: {//自定义
                if (date.equals("000000")) {
                    Toast.makeText(this, "请设置开灯时间  " + ops, Toast.LENGTH_LONG).show();
                } else {
                    open.setEnabled(true);
                    z1.setVisibility(View.VISIBLE);
                    z2.setVisibility(View.VISIBLE);
...
//        SS = hexStringToBytes(Second)[0];
        ...
        switch (judgedays(ops)) {
            case 1:
                LED_STATE_TIMER_OPEN = TimeOpen.OneDay(initweek(ops), HH, MM, SS);
            case 2:
                LED_STATE_TIMER_OPEN = TimeOpen.TwoDays(initweek(ops), HH, MM, SS);
            case 3:
                LED_STATE_TIMER_OPEN = TimeOpen.ThreeDays(initweek(ops), HH, MM, SS);
            case 4:
                LED_STATE_TIMER_OPEN = TimeOpen.FourDays(initweek(ops), HH, MM, SS);
            case 5:
                LED_STATE_TIMER_OPEN = TimeOpen.FiveDays(initweek(ops), HH, MM, SS);
            case 6:
                LED_STATE_TIMER_OPEN = TimeOpen.SixDays(initweek(ops), HH, MM, SS);
            case 7:
                LED_STATE_TIMER_OPEN = TimeOpen.EveryDay(initweek(ops), HH, MM, SS);
        }
    ...
    public void sendOrder() {
        ...
        intent.putExtra("LED_STATE_TIMER_OPEN", "周" + initweek(ops) + "   " + nowtime);
        Log.d("mytime", "周" + initweek(ops) + "   " + nowtime);
        setResult(1, intent);
        timer_open_mytimer.this.finish();
    }
    ...
    public void checkNowTime(String week) {
        Log.d("aa", "checkNowTime  nowtime    " + nowtime);
